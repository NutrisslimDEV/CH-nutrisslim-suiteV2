  ...
  function addToCart(
    ...
    if (!openside) {
      $(".simple-cart-content .cart-content-ajax-wrapper").append(
        '<div id="sideLoader"><img src="/wp-content/plugins/nutrisslim-suiteV2/assets/preloader-greybg.gif"></div>'
      );
    }
    $.ajax({
      url: cwcAjax.ajaxurl,
  ...
  if (document.body.classList.contains("is_not_mobile")) {
    ...
    tabs.forEach((tab) => {
      tab.addEventListener("mouseenter", function () {
        ...
        // Add active state to the hovered tab and its content
        this.setAttribute("aria-selected", "true");
        const activeContent = document.querySelector(
          '.e-n-tabs-content > div[data-tab-index="' + tabIndex + '"]'
        );
