...
$(function () {
  ...
  $(document.body).on("updated_checkout", function () {
    ...
    const freeShippingInput = $('input.shipping_method[value^="free_shipping"]');
    ...
    // 🔁 Translations based on cookie
    const userCountry = (document.cookie.match(/(^| )user_country=([^;]+)/) || [])[2] || 'DE';
  
    const translations = {
      DE: {
        prefix: "Ich habe gelesen und stimme der Website ",
        link: "den AGB zu"
      },
      IT: {
        prefix: "Ho letto e accetto il sito ",
        link: "termini e condizioni"
      },
      FR: {
        prefix: "J'ai lu et j'accepte le site ",
        link: "conditions générales"
      }
    };
  
    const lang = translations[userCountry.toUpperCase()] || translations['DE'];
  
    const $termsText = $(".page-id-2075 span.woocommerce-terms-and-conditions-checkbox-text");
    const $termsLink = $termsText.find("a");
  
    if ($termsText.length && $termsLink.length) {
      $termsText.html(""); // Clear existing
      $termsText.append(lang.prefix);
      $termsLink.text(lang.link);
      $termsText.append($termsLink); // Re-insert link with translated label
    }
