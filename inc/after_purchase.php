            ...
            <div id="orderInfo" class="col-8_sm-12">
                ...
                <div class="grid summary">
                    ...
                    <div class="col-8 text-right label">
                        <?php echo __('Shipping', 'nutrisslim-suiteV2') . '<span class="wo-tax"> (without VAT)</span>'; ?>
                    </div>
                    ...
                    <?php
                            foreach ( $order->get_fees() as $fee ) {
                                ...
                                echo '<div class="col-8 text-right label">' . __(esc_html( $fee_name ), 'nutrisslim-suiteV2') . '<span class="wo-tax"> (without VAT)</span>' .'</div>';
...
function thankyouContent() {
	...
	if ( is_checkout() && is_wc_endpoint_url('order-received') ) {
		...
		if ( (in_array($method, ['cod', 'stripe'])) && $enabled && ($is_afterpurchase !== 'yes') ) {
			...
			echo '<h3>' . $first_name . ', ' . __('Your order has been completed', 'nutrisslim-suiteV2') . '</h3>';
            ...
            foreach ($order->get_items() as $item_id => $item) {
                ...
                $lang_suffix = '';
                if (isset($_COOKIE['natureslang'])) {
                    $allowed_langs = ['de', 'it', 'fr'];
                    $cookie_lang = sanitize_text_field($_COOKIE['natureslang']);
                    if (in_array($cookie_lang, $allowed_langs)) {
                        $lang_suffix = '_' . $cookie_lang;
                    }
                }

                $book_field_key = 'book_file' . $lang_suffix;
                $book_file = get_field($book_field_key, $product->get_id());

                if (!$book_file && $lang_suffix !== '') {
                    // fallback to default if specific language not available
            ...
            if ($has_book_file) {
                ...
                foreach ($order->get_items() as $item_id => $item) {
                    ...
                    // Check if the product has the ACF 'book_file' field
                    $book_file = get_field('book_file', $product->get_id());
                    $cover_field_key = 'cover_image' . $lang_suffix;
$book_cover = get_field($cover_field_key, $product->get_id());
if (!$book_cover && $lang_suffix !== '') {
...
<div class="grid summary">
    ...
    <div class="col-4 text-right"><?php echo wp_kses_post($order_subtotal); ?></div>
    <div class="col-8 text-right label">
        <?php echo __('Shipping', 'nutrisslim-suiteV2') . '<span class="wo-tax"> (without VAT)</span>'; ?></div>
    <div class="col-4 text-right"><?php echo wp_kses_post($order_shipping); ?>
    </div>
