...
function trusted_get_reviews_block_a() {
    ...
    $translations = get_custom_translation_strings();
    ...
    echo '<h2 class="trusted-review-title">' . esc_html($translations['customer_reviews']) . '</h2>';
    ...
    echo '<div class="trusted-review-text">' . esc_html($translations['amazing']) . '</div>';
    ...
    foreach ($comments as $comment) {
        $rating = intval(get_comment_meta($comment->comment_ID, 'rating', true));
        $comment_content = trusted_get_language_review($comment);
...
function trusted_get_reviews_block_b() {
    ...
    */
    ...
    if ($best_review) {
        ...
        $fallback_texts = [
            'DE' => 'Der Benutzer hat keine Bewertung hinterlassen.',
            'IT' => 'L\'utente non ha lasciato alcuna recensione.',
            'FR' => 'L\'utilisateur n\'a laissé aucun avis.'
        ];
    
        // Load localized review if exists
        $review_field_key = 'review_text_' . strtolower($user_country);
        $custom_review_text = get_comment_meta($best_review->comment_ID, $review_field_key, true);
    
        // If empty, fallback to message
        if (empty(trim($custom_review_text))) {
            $custom_review_text = $fallback_texts[$user_country];
        }
        ...
        echo '<div class="trusted-rateMeta">';
        echo '<div class="stars"></div>';
        echo '<div class="scala" style="width:' . ($rating * 20) . '%"></div>';
        echo '</div>';
        
        echo '<div class="highlighted-review-comment">';
        echo '<p>' . esc_html($custom_review_text) . '</p>';
        echo '</div>'; // .highlighted-review-comment
    
        echo '<div class="trusted-divider"></div>';
        ...
        echo '</div>'; // .highlighted-review-author
    
        echo '</div>'; // .highlighted-review
    }
...
add_action('edit_comment', 'save_multilang_review_text_fields');

function save_multilang_review_text_fields($comment_id) {
    foreach (['de', 'it', 'fr'] as $lang) {
        if (isset($_POST["review_text_{$lang}"])) {
            update_comment_meta($comment_id, "review_text_{$lang}", sanitize_textarea_field($_POST["review_text_{$lang}"]));
        }
    }

    if (isset($_POST['manual_display'])) {
        update_comment_meta($comment_id, 'manual_display', '1');
    } else {
        delete_comment_meta($comment_id, 'manual_display');
    }
}
...
function trusted_get_reviews_block_c() {
    ...
    echo '<div class="trusted-review-text">' . esc_html__( 'Amazing', 'nutrisslim-suiteV2' ) . '</div>';
    $nutri_leaf_url = plugin_dir_url(__FILE__) . 'assets/nutri-leaf.svg';
    echo '<div class="trusted-review-logo"><img src="' . esc_url($nutri_leaf_url) . '" alt="logo" /></div>';
    echo '</div>';
    echo '</div>';
