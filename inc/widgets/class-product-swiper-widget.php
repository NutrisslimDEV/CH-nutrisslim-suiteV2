...
class Product_Swiper_Widget extends Widget_Base {
    ...
    protected function render() {
        ...
        foreach ( $product_list as $item ) {
            ...
            // Map of meta keys for short descriptions by country
            $short_desc_meta_keys = [
                'DE' => '_product_short_description_de',
                'IT' => '_product_short_description_it',
                'FR' => '_product_short_description_fr',
            ];
            
            // Get country-specific short description if available
            $short = '';
            if (isset($short_desc_meta_keys[$user_country])) {
                $translated = get_post_meta($pid, $short_desc_meta_keys[$user_country], true);
                if (!empty($translated)) {
                    $short = $translated;
                }
            }
            
            // Fallback to ACF or WooCommerce
            if (empty($short)) {
            ...
            // Truncate to 40 characters, remove HTML, and add ellipsis
            if (strlen($short) > 350) {
                $short = mb_substr(strip_tags($short), 0, 350) . '…';
            } else {
            ...
            echo '<div class="swiper-slide">';
                ...
                echo '<div class="content-wrapper">';
                    // Translate title based on user_country
$title_meta_keys = [
...
$translated_title = isset($title_meta_keys[$user_country])
    ? get_post_meta($pid, $title_meta_keys[$user_country], true)
    : '';

$final_title = !empty($translated_title) ? $translated_title : get_the_title($pid);

echo '<h3>' . esc_html($final_title) . '</h3>';
